...
echo =================================Epoch 0===================================
xvfb-run -a python nn_mv.py data=$DATAFILE dump=$DUMPFILE pkl=../demo/twobranch_v2.pkl lr=$7 bno=$5 mvfinetune >> garbage.txt
...
if [ "$tarn" -eq 0 ] || [ $number -eq $6 ]; then
	...
	xvfb-run -a python nn_mv.py data=$DATAFILE model=$DUMPFILE dump=$PSFILE num=$8 test >> garbage.txt
